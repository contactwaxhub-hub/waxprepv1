...
def get_wax_response(phone_number, message, message_type="text"):
    ...
    if step == "awaiting_study_time":
        ...
        return get_wax_reply(
            f"Onboarding is complete for {name}. Their goal is {goal}. Their subjects are {subjects}. They study in the {message}. Now welcome them fully in 2 short sentences, summarize what you know about them, and ask: What topic do you want to start with today?"
        )

    # NORMAL TEACHING SESSION
    increment_session(phone_number)
    name = student.get("name", "")
    ...
    profile = f"\n\nSTUDENT PROFILE: Name: {name}. Class: {class_level}. Goal: {goal}. Subjects: {subjects}. Always address them by name. You already know them well, never ask for their details again."
